...
def load_params(configs, patch_df):
    ...
    if 'TreatmentFile' in configs.keys():

        patch_treatments = pd.read_csv(
            configs['TreatmentFile'],
            names=["id", "treatment"],
            delimiter=" ",
            dtype={"id": str, "treatment": float},
        )
        patch_treatments.sort_values("id", inplace=True)


        simulated_neighbor_at_0 = dict() # flipped treatments
        simulated_self_at_0 = dict()
        ...
        counties = np.unique(patch_df.id.values)
        for county in counties:
            patch_treatments_copy = patch_treatments.copy(deep=True)
            neighbors = list(np.unique(adj_matrix[adj_matrix['src_Id'] == county].dest_Id.values))
            neighbors.remove(county)
            patch_treatments_copy.loc[patch_treatments_copy.id.isin(neighbors), 'treatment'] = 0 # flipped treatments for neighbors
            simulated_neighbor_at_0[county] = patch_treatments_copy

        T_to_be_predicted = {}
        for key, df in simulated_neighbor_at_0.items():
            T_to_be_predicted[key] = df.to_dict(orient='split')
        # Save the dictionary of DataFrames to a JSON file
        with open('flipped_treatments_to_be_predicted.json', 'w') as f:
            json.dump(T_to_be_predicted, f)

        for county in counties:
            patch_treatments_copy_2 = patch_treatments.copy(deep=True)
            t = patch_treatments_copy_2.loc[patch_treatments_copy_2['id'] == county, 'treatment'].values[0]
            if t == 0:
                patch_treatments_copy_2.loc[patch_treatments_copy_2['id'] == county, 'treatment'] = 1
            else:
                patch_treatments_copy_2.loc[patch_treatments_copy_2['id'] == county, 'treatment'] =0
            simulated_self_at_0[county] = patch_treatments_copy_2


        patch_treatments.sort_values("id", inplace=True)

        # we make the first 10 timestamps as treatment on/off, after 10 timestamps, we restore the original beta
        # when on treatment, ExposureRate is reduced at half of the level
        original_beta = float(configs.get("ExposureRate", 0.0))
        treated_beta = original_beta * 0.5
        beta_matrix = np.zeros((len(patch_df), params["T"]))
        beta_matrix = beta_matrix + original_beta
        for i in range(len(patch_treatments)):
            for j in range(params["T"]):
                if patch_treatments.iloc[i,1] == 1:
                    if j < 20:
                        beta_matrix[i,j] = treated_beta
                    else:
                        beta_matrix[i,j] = original_beta
        params["beta"] = beta_matrix

        simulated_treatments_beta_per_county = dict()
        for county in counties:
            print('flipping the treatments of neighbors for ' + str(county))
            simulated_treatment = simulated_neighbor_at_0[county]
            simulated_beta_matrix = np.zeros((len(patch_df), params["T"]))
            simulated_beta_matrix = simulated_beta_matrix + original_beta
            for i in range(len(simulated_treatment)):
                for j in range(params["T"]):
                    if simulated_treatment.iloc[i, 1] == 1:
                        if j < 20:
                            simulated_beta_matrix[i, j] = treated_beta
                        else:
                            simulated_beta_matrix[i, j] = original_beta
            simulated_treatments_beta_per_county[county] = simulated_beta_matrix
        params['simulated_treatments_beta_per_county'] = simulated_treatments_beta_per_county

        simulated_self_beta_per_county = dict()
        for county in counties:
            print('flipping the treatments of itself of ' + str(county))
            simulated_treatment = simulated_self_at_0[county]
            simulated_beta_matrix = np.zeros((len(patch_df), params["T"]))
            simulated_beta_matrix = simulated_beta_matrix + original_beta
            for i in range(len(simulated_treatment)):
                for j in range(params["T"]):
                    if simulated_treatment.iloc[i, 1] == 1:
                        if j < 20:
                            ...
                            simulated_beta_matrix[i, j] = original_beta
            simulated_self_beta_per_county[county] = simulated_beta_matrix
        params['simulated_self_beta_per_county'] = simulated_self_beta_per_county
    else:
        beta = float(configs.get("ExposureRate", 0.0))
        params["beta"] = np.full((len(patch_df), params["T"]), beta)
    # change beta here to make sure we have we are targeting the treatments to nodes, impacting the exposure rate.

    params["alpha"] = float(configs.get("InfectionRate", 0.0))
    params["gamma"] = float(configs.get("RecoveryRate", 0.0))
    if 'TreatmentFile' in configs.keys():
        logger.info(
            "Parameter: alpha=%e, original_beta=%e, treated_beta=%e, gamma=%e", params["alpha"], original_beta, treated_beta, params["gamma"]
        )
    else:
        logger.info(
...
def load_Theta(configs, patch_df):
    ...
    for src_Id, dest_Id, theta_index, flow in theta_df.itertuples(
        ...
        # We change theta here, keep self node as 0.95, and change others to 0.05/(# connected nodes - 1)
        src_Idx = patch_idx[src_Id]
        dest_Idx = patch_idx[dest_Id]
        sub_src_df = theta_df[theta_df['src_Id'] == src_Id]
        count = len(sub_src_df) - 1
        if count == 0:
            if src_Id == dest_Id:
                flow = 1
        else:
...
def do_patchsim_det_mobility_step(State_Array, patch_df, params, theta, seeds, vaxs, t, beta):
...
def patchsim_step(State_Array, patch_df, configs, params, theta, seeds, vaxs, t, stoch, beta):
    ...
    if stoch:
        ...
        if configs["Model"] == "Mobility":
            return do_patchsim_det_mobility_step(
                State_Array, patch_df, params, theta, seeds, vaxs, t, beta
            ) # theta is adj matrix
...
def epicurves_todf(configs, params, patch_df, State_Array, flipping_results, flipping_self_results):
    ...
    flipping_results: dict
        A dictionary contains the simulation results for each county if we flip its neighbor's treatments
        <FIPS: Simulated Array for States>
    Returns
    -------
    DataFrame
    ...
    final_results = dict()
    for county in flipping_results.keys():

        new_inf = flipping_results[county][2]
        ...
        out_df_simulation_at_0 = pd.DataFrame(index=index, columns=columns, data=data)
        final_results[county] = out_df_simulation_at_0

    final_flip_self_results = dict()
    for county in flipping_self_results.keys():

        new_inf = flipping_self_results[county][2]
        ...
        out_df_simulation_at_0 = pd.DataFrame(index=index, columns=columns, data=data)
        final_flip_self_results[county] = out_df_simulation_at_0



    return out_df, final_results, final_flip_self_results


def write_epicurves(configs, params, patch_df, State_Array, write_epi, return_epi, flipping_results, flipping_self_results):
    ...
    flipping_results: dict
        contains the simulation results for each county if we flip its neighbor's treatments
        <FIPS: Simulated Array for States>
    ...
    number or DataFrame
        ...
        Otherwise return the total number of people infected.
    """
    out_df, final_flipping_results, final_flipping_self_results = epicurves_todf(configs, params, patch_df, State_Array,
                                                                                 flipping_results, flipping_self_results)
    ...
    if write_epi:
        ...
        json_dict = {}
        for key, df in final_flipping_results.items():
            json_dict[key] = df.to_dict(orient='split')
        # Save the dictionary of DataFrames to a JSON file
        with open('flipping_VA_neighbor.json', 'w') as f:
            json.dump(json_dict, f)

        json_dict_2 = {}
        for key, df in final_flipping_self_results.items():
            json_dict_2[key] = df.to_dict(orient='split')
        # Save the dictionary of DataFrames to a JSON file
        with open('flipping_VA_self.json', 'w') as f:
            json.dump(json_dict_2, f)

    if return_epi:
        return out_df
    else:
        print(out_df.sum().sum())
...
def run_disease_simulation(
    ...
    if configs["NetworkType"] == "Static":
        for t in range(params["T"]):
            patchsim_step(
                State_Array, patch_df, configs, params, Theta[0], seeds, vaxs, t, stoch, beta=params['beta']
            )
        ...
        print('Simulation Done, begin to flip neighboring treatments')
        flipping_results = dict()
        for county in params['simulated_treatments_beta_per_county'].keys():
            State_Array_simulate_neighbor_at_0 = np.zeros(shape, dtype=float)
            State_Array_simulate_neighbor_at_0[0,:,:] = patch_df.pops.to_numpy()

            for t in range(params['T']):
                patchsim_step(
                    State_Array_simulate_neighbor_at_0, patch_df, configs, params, Theta[0], seeds, vaxs, t, stoch, beta=params['simulated_treatments_beta_per_county'][county]
                )
            flipping_results[county] = State_Array_simulate_neighbor_at_0
            print('Finish simulation for county '+ county +' after filpping its neighbor treatments')

        print('Simulation for neighbor flipping Done, begin to flip itself while keeping neighbors original')
        flipping_self_results = dict()
        for county in params['simulated_self_beta_per_county'].keys():
            State_Array_simulate_self_at_0 = np.zeros(shape, dtype=float)
            State_Array_simulate_self_at_0[0,:,:] = patch_df.pops.to_numpy()
            for t in range(params['T']):
                patchsim_step(
                    State_Array_simulate_self_at_0, patch_df, configs, params, Theta[0], seeds, vaxs, t, stoch, beta=params['simulated_self_beta_per_county'][county]
                )
            flipping_self_results[county] = State_Array_simulate_self_at_0
            print('Finish simulation for county '+ county +' after filpping itself treatments')
    ...
    return write_epicurves(
        configs, params, patch_df, State_Array, write_epi, return_epi, flipping_results, flipping_self_results
    )
