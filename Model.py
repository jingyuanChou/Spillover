...
class GraphSCI(nn.Module):
    ...
    def forward(self, features, treatments, edge_index):
        ...
        return results, phi_x

    def predict(self, features, flipped_treatments, edge_index):
        # This method is to predict the outcome when we flip the treatments of its neighbors
        phi_x = self.phi_x(features)
        phi_x_t = torch.mul(flipped_treatments.view(-1, 1), phi_x)
        ...
        yf_pred = torch.where(flipped_treatments > 0, y1_pred, y0_pred)
