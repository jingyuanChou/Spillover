...
def project_csc_matrix (n, adj_matrix):
    edge_num = adj_matrix.shape[0]
    rows = adj_matrix.loc[:, 'fipscounty'].values.reshape(-1)
    cols = adj_matrix.loc[:, 'fipsneighbor'].values.reshape(-1)
